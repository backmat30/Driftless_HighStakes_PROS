...
void SparkfunOTOS::updatePosition() {
  ...
  while(arduino_buffer.find(';') != std::string::npos) {
    ...
    std::string value{arduino_buffer.substr(value_start, value_end - value_start)};
  ...
  //   for (int i = 0; i < 3; i++) {
  //     char current_key{static_cast<char>(arduino_output.at(1))};
...
void SparkfunOTOS::setLocalOffset(double x_offset, double y_offset, double theta_offset) {
