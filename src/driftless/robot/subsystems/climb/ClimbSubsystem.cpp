...
ClimbSubsystem::ClimbSubsystem(std::unique_ptr<IClimb>& climb)
    : ASubsystem{ESubsystem::CLIMB}, m_climb{std::move(climb)} {}

void ClimbSubsystem::init() { m_climb->init(); }
void ClimbSubsystem::run() { m_climb->run(); }
...
void* ClimbSubsystem::state(ESubsystemState state_name) {
  ...
  switch (state_name) {
    case ESubsystemState::CLIMB_IS_CLIMBING:
      result = new bool{m_climb->isClimbing()};
      break;
    case ESubsystemState::CLIMB_ARE_PASSIVES_OUT:
      result = new bool{m_climb->arePassivesOut()};
      break;
  }
  return result;
}
